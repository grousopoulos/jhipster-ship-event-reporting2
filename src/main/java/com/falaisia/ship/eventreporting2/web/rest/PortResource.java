...
import com.falaisia.ship.eventreporting2.service.PortQueryService;
import com.falaisia.ship.eventreporting2.service.PortService;
...
/**
 * REST controller for managing {@link com.falaisia.ship.eventreporting2.domain.Port}.
 */
@RestController
@RequestMapping("/api/ports")
public class PortResource {

    private static final Logger LOG = LoggerFactory.getLogger(PortResource.class);
    ...
    public PortResource(PortService portService, PortRepository portRepository, PortQueryService portQueryService) {
    ...
    public ResponseEntity<PortDTO> createPort(@Valid @RequestBody PortDTO portDTO) throws URISyntaxException {
        ...
        portDTO = portService.save(portDTO);
        return ResponseEntity.created(new URI("/api/ports/" + portDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, portDTO.getId().toString()))
            .body(portDTO);
    }
    ...
    public ResponseEntity<PortDTO> updatePort(
        ...
        portDTO = portService.update(portDTO);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<PortDTO> getPort(@PathVariable("id") Long id) {
